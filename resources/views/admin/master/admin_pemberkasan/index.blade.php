                        ...
                        <div class="card">
                            <div class="card-header p-3">
                                <div class="d-flex align-content-center justify-content-between">
                                    <h3 class="font-weight-bold text-lg">Data Admin Pemberkasan</h3>
                                    <div class="d-flex align-items-center">
                                        @if ($permissions['tambah'])
                                            <button type="button" class="btn btn-sm btn-primary" data-toggle="modal"
                                                data-target="#modalForm">
                                                <i class="fas fa-plus"></i> Tambah Admin Pemberkasan
                                            </button>
                                        @endif
                                    </div>
                                </div>
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '[data-target="#modalForm"]', function() {
            $('#modalFormLabel').text('Tambah Admin Pemberkasan');
        });
        ...
        $(document).on('click', '.edit-button', function() {
            ...
            $.get(url, function(response) {
                if (response.status === 'success') {
                    $('#modalFormLabel').text('Edit Admin Pemberkasan');
                    ...
                    setPreview(response.data.foto, 'assets/admin_pemberkasan', 'previewFoto');
        ...
        $('#formData').on('submit', function(e) {
            ...
            let url = id ? '{{ route('admin-pemberkasan.update', ['admin_pemberkasan' => ':id']) }}'.replace(':id',
                    id) :
                '{{ route('admin-pemberkasan.store') }}';
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    audio.play();
                    let msg = id ? "Admin Pemberkasan berhasil diupdate!" :
                        "Admin Pemberkasan berhasil ditambahkan!";
